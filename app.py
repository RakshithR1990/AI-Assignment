...
def createExcelFromTextChunks(chunks):
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        print("Generated Embedded For Index:" + str(index))

    workbook.save(EXCEL_FILENAME )
    print("Text chunks exported to excel file: " + EXCEL_FILENAME)

@app.route('/get_file', methods=['GET'])
def get_file():
    filesSelected = select_files()
    if filesSelected:
        text = extract_text_from_pdfs(filesSelected)
        if text:
            createExcelFromTextChunks(divide_string_into_5(text, 10))
    if filesSelected:
        return jsonify({"filesSelected": str(filesSelected)})
    else:

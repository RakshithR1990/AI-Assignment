...
def get_embedding (text):
    api_key = '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'
...
def improve_promt (prompt, excel_file):
    ...
    for i in range(2, m_row + 1):
        ...
        vector1 = np.array(prompt_emb).reshape(1, -1)
        vector2 = np.array(db_emb_list_f).reshape(1, -1)
        cosine_sim_res = cosine_similarity(vector1, vector2)
        print ('For the Row :' + str(i) +' in excel the cosine value for prompt text is ' + str(cosine_sim_res))
